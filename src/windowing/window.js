...
export class Window {
	...
	addListeners() {
		...
		this.TOP.querySelector(".closeWindow").onmousedown = e =>
			e.stopPropagation()
	...
	aboutToSnapLeft() {
		return this.x + (this.w / 4) * 3 < 0 || event.clientX < 20
		// return event.clientX<20
	}
	aboutToSnapRight() {
		return (
			this.x > window.innerWidth - this.w / 4 ||
			event.clientX > window.innerWidth - 20
		)
		// return event.clientX> window.innerWidth - 20
	}
	aboutToSnapMaximize() {
		return this.y < 0
	}
	checkSnapHighlights(e) {
		if (this.aboutToSnapLeft()) {
			this.hideSnapHighlights()
			return leftSnapHighligt.classList.add("highlight")
		}
		if (this.aboutToSnapRight()) {
			this.hideSnapHighlights()
			return rightSnapHighligt.classList.add("highlight")
		}
		if (this.aboutToSnapMaximize()) {
			this.hideSnapHighlights()
	...
	stopDragging(e) {
		...
		if (this.aboutToSnapMaximize()) this.maximize()
		if (this.aboutToSnapLeft()) this.snapLeft()
		if (this.aboutToSnapRight()) this.snapRight()
		// minimize on bottom snap
		if (
			this.y >
			window.innerHeight - startBar.getBoundingClientRect().height * 1.5
		) {
			this.minimize()
